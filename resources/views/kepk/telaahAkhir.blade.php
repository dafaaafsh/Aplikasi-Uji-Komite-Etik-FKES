<x-Layout>
    ...
    <div class="bg-white shadow-md rounded-2xl overflow-hidden my-4">
        <div class="flex px-6 py-4 bg-gradient-to-r from-gray-900 to-gray-700 text-white justify-between rounded-t-2xl gap-x-10">
            <div>
                <h2 class="text-lg font-semibold">Daftar Penelitian Telaah Akhir </h2>
                <p class="text-sm text-sky-100">
                    Berikut adalah data semua penelitian yang telah memasuki tahapan
                    <strong>telaah akhir</strong> 
                    untuk di berikan keputusan akhir oleh 
                    <strong>KEPK</strong>
        ...
        <form method="GET" action="{{ route('kepk.telaahAkhir') }}" class="flex flex-wrap items-end justify-between gap-4 p-4 bg-blue-100 text-sm">
            ...
            <table class="min-w-full table-auto text-sm text-gray-800 divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-300">
                    ...
                    <tr class="odd:bg-white even:bg-gray-200">
                        ...
                        <td class="px-4 py-3 text-center font-semibold">
                            <button onclick="openModalKeputusan({{ $item->id }})"
                    ...
                    <tr>
                      ...
                      class="px-6 py-4 text-center text-md italic text-gray-400 font-semibold">
                        Belum ada penelitian pada telaah akhir
                      </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div id="modalKeputusan" class="fixed inset-0 bg-black/80 bg-opacity-50 items-center justify-center z-50 hidden">
        <div class="bg-white rounded-xl shadow-lg w-full max-w-xl p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">Konfirmasi Keputusan Akhir</h2>
            <p class="text-gray-700 mb-4">
                Silakan tentukan keputusan akhir berdasarkan hasil review dari tim penguji.
            </p>
    
            <form id="formKeputusan" action="{{ route('kepk.keputusan.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="protokol_id" id="keputusan_protokol_id">
    
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Keputusan Akhir</label>
                    <select name="hasil_akhir" id="keputusan" onchange="toggleDetailKeputusan()" required
                ...
                <div id="detailDiterima" class="hidden space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Penerimaan</label>
                        <select name="jenis_penerimaan" class="w-full border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">-- Pilih Jenis Penerimaan --</option>
                            <option value="Tanpa Revisi">Tanpa Revisi</option>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    <button type="button" onclick="closeModalKeputusan()" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition">
                        Batal
                    </button>
    ...
    <script>
        ...
        function openModalKeputusan(id) {
            const form = document.getElementById('formKeputusan');

            document.getElementById('keputusan_protokol_id').value = id;

            const modal = document.getElementById('modalKeputusan');
            modal.classList.remove('hidden');
            modal.classList.add('flex');
        }

        function toggleDetailKeputusan() {
            const keputusan = document.getElementById('keputusan').value;
            const detail = document.getElementById('detailDiterima');
